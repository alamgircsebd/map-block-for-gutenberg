	...
	grunt.initConfig({
		...
		copy: {
			main: {
				...
				dest: "map-block-for-gutenberg/"
			}
		},
		compress: {
			main: {
				options: {
					archive: "map-block-for-gutenberg-<%= pkg.version %>.zip",
					mode: "zip"
				},
				files: [
					{
						src: [
							"./map-block-for-gutenberg/**"
						]
		...
		makepot: {
			target: {
				options: {
					domainPath: "/",
					mainFile: "map-block-for-gutenberg.php",
					potFilename: "languages/map-block-for-gutenberg.pot",
					exclude: [
						"admin/bsf-core",
					],
					potHeaders: {
		...
		addtextdomain: {
			options: {
				textdomain: "map-block-for-gutenberg",
				updateDomains: true
			},
			target: {
				files: {
		...
		replace: {
            ...
            plugin_const: {
				src: [ "classes/class-mbfg-loader.php" ],
				overwrite: true,
				replacements: [
					{
						from: /FBFG_VER', '.*?'/g,
			...
			plugin_main: {
				src: [ "map-block-for-gutenberg.php" ],
				overwrite: true,
				replacements: [
					{
    ...
    grunt.registerTask('font-awesome', function () {
        ...
        request('https://raw.githubusercontent.com/FortAwesome/Font-Awesome/master/metadata/icons.json', function (error, response, body) {
            ...
            if (response && response.statusCode == 200) {
                ...
                fs.writeFile('blocks-config/mbfg-controls/FBFGIcon.json', JSON.stringify(fonts, null, 4), function (err) {
                    if (!err) {
