...
export default function RangeTypographyControl ( props ) {
	...
	if( "sizeTypes" in props ) {
		...
		sizeTypes = [
			{ key: "px", name: __( "px",'map-block-for-gutenberg' ) },
			{ key: "em", name: __( "em",'map-block-for-gutenberg' ) },
		]
	}

	const sizeTypesControls = (
		<ButtonGroup className="mbfg-size-type-field" aria-label={ __( "Size Type",'map-block-for-gutenberg' ) }>
			{ map( sizeTypes, ( { name, key } ) => (
				<Button
					key={ key }
					className="mbfg-size-btn"
					isSmall
