...
class TypographyControl extends Component {
	...
	render() {
		...
		if( true !== disableLineHeight ) {
			fontWeight = (
				<RangeTypographyControl
					...
					sizeMobileText = { __( "Line Height",'map-block-for-gutenberg' ) }
					sizeTabletText = { __( "Line Height",'map-block-for-gutenberg' ) }
					sizeText = { __( "Line Height",'map-block-for-gutenberg' ) }
					steps = { 0.1 }
					{ ...this.props }
				/>
			)
		...
		if( true !== disableFontSize ) {
			fontSize = (
				<RangeTypographyControl
					...
					sizeMobileText = { ( ! this.props.fontSizeLabel ) ? __( "Font Size",'map-block-for-gutenberg' ) : this.props.fontSizeLabel }
					sizeTabletText = { ( ! this.props.fontSizeLabel ) ? __( "Font Size",'map-block-for-gutenberg' ) : this.props.fontSizeLabel }
					sizeText = { ( ! this.props.fontSizeLabel ) ? __( "Font Size",'map-block-for-gutenberg' ) : this.props.fontSizeLabel }
					steps = { 0.1 }
					{ ...this.props }
				/>
			)
		...
		if( this.state !== null && this.state.showAdvancedControls === true ) {
			...
			showAdvancedFontControls = (
				<div className="mbfg-typography-advanced">
					{ fontSize }
					{ fontFamily }
					{ fontWeight }
				</div>
		...
		return (
			<div className="mbfg-typography-options">
